...
export const TableLayout = <T,>({ table }: Props<T>) => {
  return (
    <div className="relative float-left min-w-full select-none pb-[180px] px-24 mt-1">
            ...
            table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="w-full border-b border-accent-foreground h-9">
                {row.getVisibleCells().map((cell) => (
                  <td
                    key={cell.id}
                    className="flex h-full relative border-r border-accent-foreground"
